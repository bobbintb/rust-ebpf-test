...
async fn main() -> anyhow::Result<()> {
    ...
    info!("DIRT: Loading and attaching fentry 'vfs_unlink'...");
    let btf = aya::Btf::from_sys_fs()?;
    let vfs_unlink_program: &mut FEntry = ebpf.program_mut("vfs_unlink").unwrap().try_into()?;
    vfs_unlink_program.load("vfs_unlink", &btf)?;
    vfs_unlink_program.attach()?;
    info!("DIRT: fentry 'vfs_unlink' attached successfully to vfs_unlink");
