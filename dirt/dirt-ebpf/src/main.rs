...
fn try_dirt(ctx: RetProbeContext) -> Result<u32, u32> {
    ...
    info!(
        &ctx,
        "DIRT_JSON: {{\"event\":\"vfs_unlink_return\",\"pid\":{},\"tgid\":{},\"return\":{}}}",
        current_pid,
        tgid,
        ret_val
    );
    ...
    unsafe {
        bpf_printk!(
            b"DIRT: vfs_unlink RETURN - {\"pid\": %d, \"tgid\": %d, \"return\": %d}",
            current_pid,
            tgid,
            ret_val
        );
...
fn try_vfs_unlink(ctx: ProbeContext) -> Result<u32, u32> {
    ...
    let inode_nr = unsafe { (*(*inode)).i_ino };

    // Log entry information with process details in JSON format
    info!(
        &ctx,
        "DIRT_JSON: {{\"event\":\"vfs_unlink_entry\",\"pid\":{},\"tgid\":{},\"inode\":{}}}",
        current_pid,
        tgid,
        inode_nr
    );
    ...
    unsafe {
        bpf_printk!(
            b"DIRT: vfs_unlink ENTRY - {\"pid\": %d, \"tgid\": %d, \"inode\": %d}",
            current_pid,
            tgid,
            inode_nr
        );
