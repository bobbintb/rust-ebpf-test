...
#[fentry]
pub fn vfs_unlinkat_enter(ctx: FEntryContext) -> u32 {
    match try_vfs_unlinkat_enter(ctx) {
        Ok(ret) => ret,
        Err(ret) => ret,
    }
}

fn try_vfs_unlinkat_enter(ctx: FEntryContext) -> Result<u32, u32> {
    // Get process information
    let pid = aya_ebpf::helpers::bpf_get_current_pid_tgid();
    let tgid = (pid >> 32) as u32;
    let current_pid = pid as u32;
